...
re_csrf = re.compile('<input id="csrf_" type="hidden" name="csrf_" value="([^"]*)" data-formencode-ignore=\'1\' />')
re_scope = re.compile('<input type="hidden" name="scope" value="([^"]*)"/>')
re_client_id = re.compile('<input type="hidden" name="client_id" value="([^"]*)"/>')
re_redirect_uri = re.compile('<input type="hidden" name="redirect_uri" value="([^"]*)"/>')
re_response_type = re.compile('<input type="hidden" name="response_type" value="([^"]*)"/>')
...
class PyramidTestApp(unittest.TestCase):
    ...
    def test_valid_flow__registration(self):
        ...
        test_env = {
            ...
            'extra_environ_app': {'wsgi.url_scheme': 'https',
                                  'HTTP_HOST': 'app.example.com',
                                  },
            'extra_environ_authority': {'wsgi.url_scheme': 'https',
        ...
        def callback__authorization_base_url(req):
            ...
            assert req.url.startswith(oauth2_utils.OAUTH2__URL_AUTHORITY_FLOWA_AUTHORIZATION)
            (_path, _qs) = parse_request_simple(req)

            testapp_authority = test_env['testapp_authority']
            res = testapp_authority.get('/authority/oauth2/flow-a/authorization?%s' % _qs, headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=200)
            test_env['requests_session_authority'].cookies.update(testapp_authority.cookies)  # update the session with the cookies from the response
        ...
        def callback__authorization_base_url_post(req):
            ...
            testapp_authority = test_env['testapp_authority']
            res = testapp_authority.post('/authority/oauth2/flow-a/authorization', payload, headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=302)
            test_env['requests_session_authority'].cookies.update(testapp_authority.cookies)  # update the session with the cookies from the response
            ...
            assert _uri_redirect.startswith(oauth2_model.OAUTH2__URL_APP_FLOW_REGISTER_CALLBACK)
        ...
        def callback__token_url_post(req):
            ...
            _headers = string_headers(req.headers)  # these can end up being unicode in tests
            testapp_authority = test_env['testapp_authority']
            res = testapp_authority.post('/authority/oauth2/flow-a/token', payload, headers=_headers, extra_environ=test_env['extra_environ_authority'], status=200)
            test_env['requests_session_authority'].cookies.update(testapp_authority.cookies)  # update the session with the cookies from the response
        ...
        def callback__app_callback(req):
            ...
            res = testapp_app.get('/application/flow-register/authorized-callback?%s' % _qs, headers=req.headers, extra_environ=test_env['extra_environ_app'], status=303)
            test_env['requests_session_app'].cookies.update(testapp_app.cookies)  # update the session with the cookies from the response
        ...
        def callback__app_callback_success(req, test_env=test_env, ):
            ...
            testapp_app = test_env['testapp_app']
            res = testapp_app.get('/application/flow-register/authorized-callback-success?%s' % _qs, headers=req.headers, extra_environ=test_env['extra_environ_app'], status=200)
            test_env['requests_session_app'].cookies.update(testapp_app.cookies)  # update the session with the cookies from the response
        ...
        def callback__app_fetch_protected_resource(req, test_env=test_env, ):
            ...
            testapp_app = test_env['testapp_app']
            res = testapp_app.get('/application/account/fetch-protected-resource', headers=req.headers, extra_environ=test_env['extra_environ_app'], status=200)
            test_env['requests_session_app'].cookies.update(testapp_app.cookies)  # update the session with the cookies from the response
        ...
        def callback__authority_protected_resource(req, test_env=test_env, ):
            ...
            testapp_authority = test_env['testapp_authority']
            _headers = string_headers(req.headers)  # these can end up being unicode in tests
            res = testapp_authority.get('/authority/oauth2/protected_resource', headers=_headers, extra_environ=test_env['extra_environ_authority'], status=200)
        ...
        def callback__app_refresh_token(req, test_env=test_env, ):
            ...
            testapp_app = test_env['testapp_app']
            res = testapp_app.get('/application/account/refresh-token', headers=req.headers, extra_environ=test_env['extra_environ_app'], status=200)
            test_env['requests_session_app'].cookies.update(testapp_app.cookies)  # update the session with the cookies from the response
        ...
        def callback__app_revoke_token(req, test_env=test_env, ):
            ...
            testapp_app = test_env['testapp_app']
            res = testapp_app.get('/application/account/revoke-token', headers=req.headers, extra_environ=test_env['extra_environ_app'], status=200)
            test_env['requests_session_app'].cookies.update(testapp_app.cookies)  # update the session with the cookies from the response
        ...
        def callback__url_revoke_token_post(req):
            ...
            testapp_authority = test_env['testapp_authority']
            res = testapp_authority.post('/authority/oauth2/revoke_token', payload, headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=200)
            test_env['requests_session_authority'].cookies.update(testapp_authority.cookies)  # update the session with the cookies from the response
        ...
        with responses.RequestsMock() as rsps:
            ...
            # )
            rsps.add_callback(
                responses.POST, oauth2_model.OAUTH2__URL_AUTHORITY_FLOWA_TOKEN,  # https://authority.example.com/authority/oauth2/flow-a/token
                callback=callback__token_url_post,
            )
            rsps.add_callback(
                responses.GET, oauth2_model.OAUTH2__URL_APP_FETCH_PROTECTED_RESOURCE,  # https://app.example.com/application/account/fetch-protected-resource
                callback=callback__app_fetch_protected_resource,
            )
            rsps.add_callback(
                responses.GET, oauth2_model.OAUTH2__URL_AUTHORITY_PROTECTED_RESOURCE,  # https://authority.example.com/authority/oauth2/protected_resource
                callback=callback__authority_protected_resource,
            )
            rsps.add_callback(
                responses.POST, oauth2_utils.OAUTH2__URL_AUTHORITY_REVOKE_TOKEN,  # 'https://authority.example.com/authority/oauth2/revoke_token',
                callback=callback__url_revoke_token_post,
            )
            rsps.add_callback(
                responses.GET, oauth2_model.OAUTH2__URL_APP_REFRESH_TOKEN,  # https://app.example.com/application/account/refresh-token
                callback=callback__app_refresh_token,
            )
            rsps.add_callback(
                responses.GET, oauth2_model.OAUTH2__URL_APP_REVOKE_TOKEN,  # https://app.example.com/application/account/revoke-token
            ...
            # User visit
            res = self.testapp_authority.get('/authority/account/login-form', extra_environ=test_env['extra_environ_authority'], status=200)
            assert res.text == 'authority|login-form'
            test_env['requests_session_authority'].cookies.update(self.testapp_authority.cookies)  # update the session with the cookies from the response

            # User visit
            res = self.testapp_authority.get('/authority/account/login-submit', extra_environ=test_env['extra_environ_authority'], status=303)
            test_env['requests_session_authority'].cookies.update(self.testapp_authority.cookies)  # update the session with the cookies from the response
            assert res.text == """303 See Other\n\nThe resource has been moved to /authority/account/home; you should be redirected automatically.\n\n"""

            # User visit
            res = self.testapp_authority.get('/authority/account/home', extra_environ=test_env['extra_environ_authority'], status=200)
            test_env['requests_session_authority'].cookies.update(self.testapp_authority.cookies)  # update the session with the cookies from the response
            assert res.text == ('authority|home|user=%s' % oauth2_model.USERID_ACTIVE__AUTHORITY)
            ...
            #
            res = self.testapp_app.get('/application/flow-register', extra_environ=test_env['extra_environ_app'], status=303)
            test_env['requests_session_app'].cookies.update(self.testapp_app.cookies)  # update the session with the cookies from the response
            ...
            res = self.testapp_app.get('/application/flow-register/oauth2/start', extra_environ=test_env['extra_environ_app'], status=303)
            test_env['requests_session_app'].cookies.update(self.testapp_app.cookies)  # update the session with the cookies from the response
            assert 'Location' in res.headers
            url_authform = res.headers['Location']
            assert res.headers['Location'].startswith(oauth2_utils.OAUTH2__URL_AUTHORITY_FLOWA_AUTHORIZATION)
            ...
            resAuthInbound = self.testapp_authority.get(url_authform_local, extra_environ=test_env['extra_environ_authority'])

            assert '<form action="/authority/oauth2/flow-a/authorization" method="POST" id="app-action-authorize">' in resAuthInbound.text
            ...
            resAuthSubmit = self.testapp_authority.post("/authority/oauth2/flow-a/authorization", payload, extra_environ=test_env['extra_environ_authority'], status=302)
            test_env['requests_session_authority'].cookies.update(self.testapp_authority.cookies)  # update the session with the cookies from the response
            assert 'Location' in resAuthSubmit.headers
            url_callback = resAuthSubmit.headers['Location']
            assert url_callback.startswith(oauth2_model.OAUTH2__URL_APP_FLOW_REGISTER_CALLBACK)

            # user visits the app callback, is redirected to callback-success
            qs = url_callback.split('?')[1]
            url_callback_local = '/application/flow-register/authorized-callback?%s' % qs
            resAppCallback = self.testapp_app.get(url_callback_local, payload, extra_environ=test_env['extra_environ_app'], status=303)
            test_env['requests_session_app'].cookies.update(self.testapp_app.cookies)  # update the session with the cookies from the response
            assert 'Location' in resAppCallback.headers
            url_callback_success = resAppCallback.headers['Location']
            assert url_callback_success.startswith(oauth2_model.OAUTH2__URL_APP_FLOW_REGISTER_CALLBACK_SUCCESS)

            url_callback_success_local = '/application/flow-register/authorized-callback-success'
            resAppCallbackSuccess = self.testapp_app.get(url_callback_success_local, payload, extra_environ=test_env['extra_environ_app'], status=200)
            test_env['requests_session_app'].cookies.update(self.testapp_app.cookies)  # update the session with the cookies from the response
            assert resAppCallbackSuccess.text == 'example_app|authorized-callback-success|user=%s' % oauth2_model.USERID_ACTIVE__APPLICATION
            ...
            resProtectedAttempt = test_env['requests_session_app'].get(oauth2_model.OAUTH2__URL_APP_FETCH_PROTECTED_RESOURCE)
            ...
            resProtectedAttempt = test_env['requests_session_app'].get(oauth2_model.OAUTH2__URL_APP_REFRESH_TOKEN)
            assert resProtectedAttempt.status_code == 200
            ...
            resProtectedAttempt = test_env['requests_session_app'].get(oauth2_model.OAUTH2__URL_APP_REVOKE_TOKEN)
            assert resProtectedAttempt.status_code == 200
            assert resProtectedAttempt.text == 'revoked_token'

            # ensure logout, just to be safe
            res = self.testapp_authority.get('/authority/account/logout', extra_environ=test_env['extra_environ_authority'], status=303)
            assert res.text == """303 See Other\n\nThe resource has been moved to /authority/account/login-form; you should be redirected automatically.\n\n"""

            res = self.testapp_authority.get('/authority/account/home', extra_environ=test_env['extra_environ_authority'], status=303)
            assert res.text == """303 See Other\n\nThe resource has been moved to /authority/account/login-form; you should be redirected automatically.\n\n"""
    ...
    def test_valid_flow__client_credentials_grant(self):
        ...
        def callback__url_obtain_token__post(req):
            ...
            testapp_authority = test_env['testapp_authority']
            res = testapp_authority.post('/authority/oauth2/flow-b/obtain_token', payload, headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=200)
            test_env['requests_session_authority'].cookies.update(testapp_authority.cookies)  # update the session with the cookies from the response
        ...
        def callback__url_obtain_token_alt__post(req):
            ...
            testapp_authority = test_env['testapp_authority']
            res = testapp_authority.post('/authority/oauth2/flow-b/obtain_token_alt', payload, headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=200)
            test_env['requests_session_authority'].cookies.update(testapp_authority.cookies)  # update the session with the cookies from the response
        ...
        def callback__url_token_limited__post(req):
            ...
            testapp_authority = test_env['testapp_authority']
            res = testapp_authority.post('/authority/oauth2/flow-c/token_limited', payload, headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=400)
            test_env['requests_session_authority'].cookies.update(testapp_authority.cookies)  # update the session with the cookies from the response
        ...
        with responses.RequestsMock() as rsps:
            ...
            rsps.add_callback(
                responses.POST, apiClient._url_obtain_token,  # https://authority.example.com/authority/oauth2/flow-b/obtain_token
                callback=callback__url_obtain_token__post,
            )

            rsps.add_callback(
                responses.POST, apiClient._url_obtain_token_alt,  # https://authority.example.com/authority/oauth2/flow-b/obtain_token_alt
                callback=callback__url_obtain_token_alt__post,
            )

            rsps.add_callback(
                responses.POST, apiClient._url_token_limited,  # https://authority.example.com/authority/oauth2/flow-c/token_limited
            ...
            # User visit
            res = self.testapp_authority.get('/authority/oauth2/flow-b/obtain_token', status=400)
            assert "Only `HTTPS` connections are accepted" in res.text
            res = self.testapp_authority.get('/authority/oauth2/flow-b/obtain_token', extra_environ=test_env['extra_environ_authority'], status=400)
            assert "Only `POST` is accepted." in res.text

            # can we revoke the token?
            token_result = apiClient.revoke_access_token(token=token_result.get('access_token'))
            ...
            try:
                ...
                assert exc.original_response.text == '{"error": "unsupported_grant_type"}'
