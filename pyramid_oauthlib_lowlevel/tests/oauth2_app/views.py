...
class Authority_Oauth2_FlowShared_API_Public(Handler):
    ...
    def protected_resource(self):
        ...
        if LOG_ROUTE:
            log.debug("authority:oauth2:protected_resource %s", get_csrf_token(self.request))
...
class Authority_Oauth2_FlowA_API_Public(Handler):
    ...
    def authorization(self):
        ...
        if LOG_ROUTE:
            log.debug("authority:oauth2:flow_a:authorization %s", get_csrf_token(self.request))
        ...
        try:
            ...
            if self.request.method == 'GET':
                ...
                validity_dict = oauth2Provider.endpoint__validate_authorization_request()
    ...
    def _authorization__print(self):
        """print the form"""
        return render_to_response("pyramid_oauthlib_lowlevel:tests/oauth2_app/templates/authorize-form.mako", {}, self.request)
...
class Authority_Oauth2_FlowB_API_Public(Handler):
    ...
    def obtain_token(self):
        ...
        if LOG_ROUTE:
            log.debug("authority:oauth2:flow_b:obtain_token %s", get_csrf_token(self.request))
        try:
            oauth2Provider = new_oauth2Provider(self.request)
    ...
    @view_config(route_name="authority:oauth2:flow_b:obtain_token_alt", renderer="string")
    def obtain_token_alt(self):
        ...
        """
        if DEBUG_ROUTE:
            print("authority:oauth2:flow_b:obtain_token_alt", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("authority:oauth2:flow_b:obtain_token_alt %s", get_csrf_token(self.request))
        try:
            ...
            credentials = {'username': credentials.username, 'password': credentials.password, } if credentials else {}
...
class Authority_Oauth2_Flowc_API_Public(Handler):
    ...
    def obtain_token(self):
        ...
        if LOG_ROUTE:
            log.debug("authority:oauth2:flow_c:token_limited %s", get_csrf_token(self.request))
        try:
...
class ExampleApp_User_AccountViews(Handler):
    ...
    def account_login_submit(self):
        ...
        if LOG_ROUTE:
            log.debug("application:account:login-submit %s", get_csrf_token(self.request))
        if DEBUG_USERID:
            print("application:account:login-submit", self.request.active_useraccount_id)
    ...
    @view_config(route_name="application:account:fetch-protected-resource", renderer="string")
    def fetch_protected_resource(self):
        ...
        """
        if DEBUG_ROUTE:
            print("application:account:fetch-protected-resource", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("application:account:fetch-protected-resource %s", get_csrf_token(self.request))
        if not self.request.active_useraccount_id:
            return HTTPSeeOther('/application/account/login-form')

        # what is our token?
        clientToken = self.request.dbSession.query(Developer_OAuth2Client_BearerToken)\
            .filter(Developer_OAuth2Client_BearerToken.useraccount_id == self.request.active_useraccount_id,
                    Developer_OAuth2Client_BearerToken.original_grant_type == 'authorization_code',
        ...
        token_dict = {'access_token': clientToken.access_token,
                      'token_type': 'Bearer',
                      }
        sess = OAuth2Session(client_id=OAUTH2__APP_KEY, token=token_dict)
        resp = sess.request('GET', oauth2_utils.OAUTH2__URL_AUTHORITY_PROTECTED_RESOURCE)
    ...
    def refresh_token(self):
        ...
        if LOG_ROUTE:
            log.debug("application:account:refresh-token %s", get_csrf_token(self.request))
        if not self.request.active_useraccount_id:
            return HTTPSeeOther('/application/account/login-form')
        ...
        # what is our token?
        clientToken = self.request.dbSession.query(Developer_OAuth2Client_BearerToken)\
            .filter(Developer_OAuth2Client_BearerToken.useraccount_id == USERID_ACTIVE__APPLICATION,
                    Developer_OAuth2Client_BearerToken.original_grant_type == 'authorization_code',
                    Developer_OAuth2Client_BearerToken.is_active == True,  # noqa
                    )\
            .first()
        ...
        sess = OAuth2Session(client_id=OAUTH2__APP_KEY, token=token_dict)
        newToken_dict = sess.refresh_token(oauth2_utils.OAUTH2__URL_AUTHORITY_FLOWA_TOKEN, **extra)
        ...
        newToken_db.timestamp_expires = newToken_db.timestamp_created + datetime.timedelta(seconds=newToken_dict['expires_in'])  # or use newToken_dict['expires_at]
        ...
        _clientTokens = self.request.dbSession.query(Developer_OAuth2Client_BearerToken)\
            .filter(Developer_OAuth2Client_BearerToken.useraccount_id == USERID_ACTIVE__APPLICATION,
                    Developer_OAuth2Client_BearerToken.original_grant_type == 'authorization_code',
                    Developer_OAuth2Client_BearerToken.is_active == True,  # noqa
                    )\
        ...
        _serverTokens = self.request.dbSession.query(Developer_OAuth2Server_BearerToken)\
            .filter(Developer_OAuth2Server_BearerToken.useraccount_id == USERID_ACTIVE__AUTHORITY,
                    Developer_OAuth2Server_BearerToken.original_grant_type == 'authorization_code',
        ...
        oauth2_utils.CustomValidator.rotate_refresh_token = oauth2_utils.CustomValidator._rotate_refresh_token__False

        # grab this data to send upstream
        token_dict = {'access_token': newToken_db.access_token,
                      'token_type': 'Bearer',
        ...
        sess = OAuth2Session(client_id=OAUTH2__APP_KEY, token=token_dict)
        newToken_dict2 = sess.refresh_token(oauth2_utils.OAUTH2__URL_AUTHORITY_FLOWA_TOKEN, **extra)
        if not isinstance(newToken_dict, OAuth2Token):
            raise ValueError("did not load an `OAuth2Token``")
        ...
        newToken_db2.timestamp_expires = newToken_db.timestamp_created + datetime.timedelta(seconds=newToken_dict2['expires_in'])  # or use newToken_dict['expires_at]
        ...
        oauth2_utils.CustomValidator.rotate_refresh_token = oauth2_utils.CustomValidator._rotate_refresh_token__True
    ...
    def revoke_token(self):
        ...
        if LOG_ROUTE:
            log.debug("application:account:revoke-token %s", get_csrf_token(self.request))
        if not self.request.active_useraccount_id:
            return HTTPSeeOther('/application/account/login-form')
        if self.request.active_useraccount_id != USERID_ACTIVE__APPLICATION:
            raise ValueError("not the expected user!")
        ...
        _serverToken = self.request.dbSession.query(Developer_OAuth2Server_BearerToken)\
            .filter(Developer_OAuth2Server_BearerToken.useraccount_id == USERID_ACTIVE__AUTHORITY,
                    Developer_OAuth2Server_BearerToken.original_grant_type == 'authorization_code',
                    Developer_OAuth2Server_BearerToken.is_active == False,  # noqa
                    Developer_OAuth2Server_BearerToken.access_token == clientToken.access_token,
                    )\
            .first()
...
class ExampleApp_FlowRegister(Handler):
    ...
    def oauth2_start(self):
        if DEBUG_ROUTE:
            print("application:flow-register:oauth2:start", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("application:flow-register:oauth2:start %s", get_csrf_token(self.request))
        if DEBUG_USERID:
            print("application:account:oauth2:start", self.request.active_useraccount_id)

        if self.request.active_useraccount_id:
            return HTTPSeeOther('/application/account/home')

        authClient = OAuth2Session(OAUTH2__APP_KEY, redirect_uri=oauth2_utils.OAUTH2__URL_APP_FLOW_REGISTER_CALLBACK)

        # Redirect user to the App for authorization
        authorization_url, state = authClient.authorization_url(oauth2_utils.OAUTH2__URL_AUTHORITY_FLOWA_AUTHORIZATION)

        return HTTPSeeOther(authorization_url)

    @view_config(route_name="application:flow-register:oauth2:authorized-callback", renderer="string")
    def authorized_callback(self):
        if DEBUG_ROUTE:
            print("application:flow-register:oauth2:authorized-callback", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("application:flow-register:oauth2:authorized-callback %s", get_csrf_token(self.request))
        ...
        try:
            sess = OAuth2Session(client_id=OAUTH2__APP_KEY,
                                 state=_state,
                                 redirect_uri=oauth2_utils.OAUTH2__URL_APP_FLOW_REGISTER_CALLBACK,
                                 )
            resp = sess.fetch_token(oauth2_utils.OAUTH2__URL_AUTHORITY_FLOWA_TOKEN,
                                    client_id=OAUTH2__APP_KEY,
                                    client_secret=OAUTH2__APP_SECRET,
            ...
            clientToken.timestamp_expires = clientToken.timestamp_created + datetime.timedelta(seconds=resp['expires_in'])
            self.request.dbSession.add(clientToken)
            self.request.dbSession.flush()

            return HTTPSeeOther(location='/application/flow-register/authorized-callback-success')

        except:
            raise

    @view_config(route_name="application:flow-register:oauth2:authorized-callback-success", renderer="string")
    def authorized_callback_success(self):
        if DEBUG_ROUTE:
            print("application:flow-register:oauth2:authorized-callback-success", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("application:flow-register:oauth2:authorized-callback-success %s", get_csrf_token(self.request))
        if not self.request.active_useraccount_id:
            return HTTPSeeOther('/application/account/login-form')
        return 'example_app|authorized-callback-success|user=%s' % self.request.active_useraccount_id
