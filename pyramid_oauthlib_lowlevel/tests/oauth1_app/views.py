...
class Authority_Oauth1_FlowShared_API_Public(Handler):
    ...
    def authorize(self):
        ...
        try:
            ...
            oauth1_TokenRequest = self.request.dbSession.query(Developer_oAuth1Server_TokenRequest)\
                .filter(Developer_oAuth1Server_TokenRequest.oauth_token == oauth1_data['credentials']['resource_owner_key'],
            ...
            return render_to_response("pyramid_oauthlib_lowlevel:tests/oauth1_app/templates/authorize-error.mako", {}, self.request)

    def _authorize_print(self):
        """print the form"""
        return render_to_response("pyramid_oauthlib_lowlevel:tests/oauth1_app/templates/authorize-form.mako", {}, self.request)
...
class ExampleApp_FlowRegister(Handler):
    ...
    def oauth1_start(self):
        if DEBUG_ROUTE:
            print("application:flow-register:oauth1:start", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("application:flow-register:oauth1:start %s", get_csrf_token(self.request))
        if DEBUG_USERID:
            print("application:account:oauth1:start", self.request.active_useraccount_id)
        ...
        self.request.session['3rdparty-app_oauth']['next'] = self.request.params.get('next')

        app_data = get_ApiExampleAppData()
        apiClient = CustomApiClient(app_key = app_data['client_key'],
                                    app_secret = app_data['client_secret'],
                                    client_args = {'verify': False, },
                                    )
        try:
            auth_props = apiClient.get_authentication_tokens(callback_url=app_data['callback_uri'])
            self.request.session['3rdparty-app_oauth']['auth_url'] = auth_props['auth_url']
            self.request.session['3rdparty-app_oauth']['oauth_token'] = auth_props['oauth_token']
            self.request.session['3rdparty-app_oauth']['oauth_token_secret'] = auth_props['oauth_token_secret']
    ...
    @view_config(route_name="application:flow-register:oauth1:authorized-callback", renderer="string")
    def registration_authorized_callback(self):
        if DEBUG_ROUTE:
            print("application:flow-register:oauth1:authorized-callback", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("application:flow-register:oauth1:authorized-callback %s", get_csrf_token(self.request))

        if DEBUG_USERID:
            print("application:account:register:oauth1:authorized-callback", self.request.active_useraccount_id)
        ...
        if (_oauth_token is None) or (_oauth_token_secret is None):
            raise ApiError("we could not link your authorization session data correctly.")
        ...
        newGrant._realms = authorized['oauth_authorized_realms'] if 'oauth_authorized_realms' in authorized else ''
    ...
    @view_config(route_name="application:flow-register:oauth1:authorized-callback-success", renderer="string")
    def registration_authorized_callback_success(self):
        if DEBUG_ROUTE:
            print("application:flow-register:oauth1:authorized-callback-success", get_csrf_token(self.request))
        if LOG_ROUTE:
            log.debug("application:flow-register:oauth1:authorized-callback-success|user=%s", get_csrf_token(self.request))
        if DEBUG_USERID:
            print("application:account:register:oauth1:authorized-callback-success", self.request.active_useraccount_id)
        if not self.request.active_useraccount_id:
            return HTTPSeeOther('/application/account/login-form')

        # so let's just check to ensure we have the right number of active tokens (Client)
        _clientTokens = self.request.dbSession.query(Developer_oAuth1Client_TokenAccess)\
            .filter(Developer_oAuth1Client_TokenAccess.useraccount_id == USERID_ACTIVE__APPLICATION,
        ...
        _serverTokens = self.request.dbSession.query(Developer_oAuth1Server_TokenAccess)\
            .filter(Developer_oAuth1Server_TokenAccess.useraccount_id == USERID_ACTIVE__AUTHORITY,
        ...
        assert _serverTokens[0].oauth_token_secret == _clientTokens[0].oauth_token_secret
        assert _serverTokens[0].oauth_token == _clientTokens[0].oauth_token
        assert _serverTokens[0]._realms == _clientTokens[0]._realms

        # yep, we good
        return 'application|register|authorized-callback-success|user=%s' % self.request.active_useraccount_id
