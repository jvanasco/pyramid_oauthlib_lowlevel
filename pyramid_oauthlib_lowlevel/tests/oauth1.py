...
class PyramidTestApp(unittest.TestCase):
    ...
    def test_invalids(self):
        ...
        res = self.testapp_authority.get('/authority/oauth1/request_token', extra_environ=test_env['extra_environ_authority'], status=400)
        ...
        assert res.text == 'error=invalid_request&error_description=Missing+mandatory+OAuth+parameters.'

        res = self.testapp_authority.get('/authority/oauth1/access_token', extra_environ=test_env['extra_environ_authority'], status=400)
        # assert res.text == 'error=oauth1_error&error_description=Exception+caught+while+processing+request'  # useful for debugging a broken backend
        assert res.text == 'error=invalid_request&error_description=Missing+mandatory+OAuth+parameters.'
        ...
        res = self.testapp_authority.get('/authority/oauth1/authorize', extra_environ=test_env['extra_environ_authority'], status=200)
    ...
    def test_valid_flow__registration(self):
        ...
        def callback__request_token(req, test_env=test_env):
            ...
            with IsolatedTestapp(test_env['testapp_authority']) as testapp:
                res = testapp.get('/authority/oauth1/request_token', headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=200)
        ...
        def callback__authenticate_get(req, test_env=test_env, ):
            ...
            res = testapp.get('/authority/oauth1/authorize?oauth_token=%s' % qs['oauth_token'], headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=200)
            test_env['requests_session_authority'].cookies.update(testapp.cookies)  # update the session with the cookies from the response
        ...
        def callback__authenticate_post(req, test_env=test_env, ):
            ...
            testapp = test_env['testapp_authority']
            res = testapp.post('/authority/oauth1/authorize', payload, headers=req.headers, extra_environ=test_env['extra_environ_authority'], status=302)
            test_env['requests_session_authority'].cookies.update(testapp.cookies)  # update the session with the cookies from the response
        ...
        def callback__callback(req, test_env=test_env, ):
            ...
            res = testapp.get('/application/flow-register/authorized-callback?%s' % _qs, headers=req.headers, extra_environ=test_env['extra_environ_app'], status=303)
            test_env['requests_session_app'].cookies.update(testapp.cookies)  # update the session with the cookies from the response
        ...
        def callback__access_token(req, test_env=test_env,):
            ...
            with IsolatedTestapp(test_env['testapp_authority']) as testapp:
                ...
                res = testapp.get('/authority/oauth1/access_token', headers=_headers, extra_environ=test_env['extra_environ_authority'], status=200)
        ...
        def callback__callback_success(req, test_env=test_env, ):
            ...
            _headers = string_headers(req.headers)  # these can end up being unicode in tests
            res = testapp.get('/application/flow-register/authorized-callback-success?%s' % _qs, headers=_headers, extra_environ=test_env['extra_environ_app'], status=200)
            test_env['requests_session_application'].cookies.update(testapp.cookies)  # update the session with the cookies from the response
        ...
        with responses.RequestsMock() as rsps:
            ...
            rsps.add_callback(
                responses.GET, oauth1_utils.CustomApiClient.OAUTH1_SERVER_REQUEST_TOKEN,  # /authority/oauth1/request_token
                callback=callback__request_token,
            )
            rsps.add_callback(
                responses.GET, oauth1_utils.CustomApiClient.OAUTH1_SERVER_ACCESS_TOKEN,  # /authority/oauth1/access_token
            ...
            test_env['requests_session_authority'].cookies.update(self.testapp_authority.cookies)  # update the session with the cookies from the response
            assert res.text == 'authority|home|user=%s' % oauth1_model.USERID_ACTIVE__AUTHORITY
            ...
            res = self.testapp_app.get('/application/flow-register/oauth1/start', extra_environ=test_env['extra_environ_app'], status=303)
            test_env['requests_session_app'].cookies.update(self.testapp_app.cookies)  # update the session with the cookies from the response
            assert 'Location' in res.headers
            url_auth = res.headers['Location']
            assert res.headers['Location'].startswith(OAUTH1__URL_AUTHORITY_AUTHENTICATE)
            ...
            resAuthInbound = self.testapp_authority.get(url_auth_local, extra_environ=test_env['extra_environ_authority'])
            assert '<form action="/authority/oauth1/authorize" method="POST" id="app-action-authorize">' in resAuthInbound.text
            ...
            resAuthApprove = self.testapp_authority.post('/authority/oauth1/authorize', payload, extra_environ=test_env['extra_environ_authority'], status=302)
            test_env['requests_session_authority'].cookies.update(self.testapp_authority.cookies)  # update the session with the cookies from the response
            ...
            resAuthCallback = self.testapp_app.get(url_callback_local, extra_environ=test_env['extra_environ_app'], status=303)
            ...
            assert url_callback_success.startswith(OAUTH1__URL_APP_FLOW_REGISTER_CALLBACK_SUCCESS)
            assert len(url_callback_success.split('?')) == 1
            url_callback_success_local = '/application/flow-register/authorized-callback-success'
            resAuthCallbackSuccess = self.testapp_app.get(url_callback_success_local, extra_environ=test_env['extra_environ_app'], status=200)
            assert resAuthCallbackSuccess.text == 'application|register|authorized-callback-success|user=%s' % oauth1_model.USERID_ACTIVE__APPLICATION
...
class TestOauth1Faked(unittest.TestCase):
    ...
    def test_request_token(self):
        ...
        req = new_req_session()
        req.current_route_url(uri=oauth1_utils.CustomApiClient.OAUTH1_SERVER_REQUEST_TOKEN)
        provider = oauth1_utils.new_oauth1Provider(req)
        result = provider.endpoint__request_token(dbSessionCommit=req.dbSession)
        assert result.status_code == 400
        assert result.text == "error=invalid_request&error_description=Missing+mandatory+OAuth+parameters."

        req.headers = {'Authorization': OAUTH_EXAMPLE_AUTH, }
        provider = oauth1_utils.new_oauth1Provider(req)
        result = provider.endpoint__request_token(dbSessionCommit=req.dbSession)
        assert result.status_code == 400
        assert result.text == "error=invalid_request&error_description=Timestamp+given+is+invalid%2C+differ+from+allowed+by+over+600+seconds."
        ...
        req.headers = {'Authorization': OAUTH_EXAMPLE_AUTH.replace('1533856374', oauth1_utils.oauth_time_now()), }
        provider = oauth1_utils.new_oauth1Provider(req)
        result = provider.endpoint__request_token(dbSessionCommit=req.dbSession)
        assert result.status_code == 400
    ...
    def test_request_token_backend_failure(self):
        ...
        assert result.text == u'error=internal_system_failure&error_description=Internal+System+Failure'
