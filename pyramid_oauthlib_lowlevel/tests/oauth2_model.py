...
OAUTH2__URL_AUTHORITY_FLOWA_AUTHORIZATION = 'https://authority.example.com/authority/oauth2/flow-a/authorization'
OAUTH2__URL_AUTHORITY_FLOWA_TOKEN = 'https://authority.example.com/authority/oauth2/flow-a/token'
OAUTH2__URL_AUTHORITY_FLOWB_TOKEN = 'https://authority.example.com/authority/oauth2/flow-b/obtain_token'
OAUTH2__URL_AUTHORITY_FLOWB_TOKEN_ALT = 'https://authority.example.com/authority/oauth2/flow-b/obtain_token_alt'
OAUTH2__URL_AUTHORITY_FLOWC_TOKEN_LIMITED = 'https://authority.example.com/authority/oauth2/flow-c/token_limited'
OAUTH2__URL_AUTHORITY_REVOKE_TOKEN = 'https://authority.example.com/authority/oauth2/revoke_token'
OAUTH2__URL_AUTHORITY_PROTECTED_RESOURCE = 'https://authority.example.com/authority/oauth2/protected_resource'

OAUTH2__URL_APP_FLOW_REGISTER_CALLBACK = 'https://app.example.com/application/flow-register/authorized-callback'
OAUTH2__URL_APP_FLOW_REGISTER_CALLBACK_SUCCESS = 'https://app.example.com/application/flow-register/authorized-callback-success'
OAUTH2__URL_APP_FETCH_PROTECTED_RESOURCE = 'https://app.example.com/application/account/fetch-protected-resource'
OAUTH2__URL_APP_REFRESH_TOKEN = 'https://app.example.com/application/account/refresh-token'
OAUTH2__URL_APP_REVOKE_TOKEN = 'https://app.example.com/application/account/revoke-token'
...
class DeveloperApplication(Base):
    ...
    _default_scopes = sa.Column(sa.Unicode(200), nullable=True)  # The value of the scope parameter is expressed as a list of space- delimited, case-sensitive strings.
    is_confidential = sa.Column(sa.Boolean, nullable=True, default=False)  # Flask-oauthlib uses this to implement some RFC details see ``OAuth2RequestValidator.client_authentication_required```
...
class Developer_OAuth2Server_BearerToken(Base):
    ...
    developer_application_id = sa.Column(sa.Integer, sa.ForeignKey("developer_application.id"), nullable=False)
    is_active = sa.Column(sa.Boolean, nullable=True, default=True)
    access_token = sa.Column(sa.Unicode(255), nullable=True, unique=True)  # `payload:access_token`
    refresh_token = sa.Column(sa.Unicode(255), nullable=True, unique=False)  # `payload:refresh_token`  # not unique because we might recycle these; will be NULL if this is a `client_credentials` grant | https://tools.ietf.org/html/rfc6749#section-4.4.3 states "A refresh token SHOULD NOT be included."
    token_type = sa.Column(sa.Unicode(32), nullable=False, default='Bearer')  # `payload:token_type`, unnecessary
    timestamp_created = sa.Column(sa.DateTime, nullable=False)
    timestamp_expires = sa.Column(sa.DateTime, nullable=False)  # based on `payload:expires_in`
    timestamp_revoked = sa.Column(sa.DateTime, nullable=True)
    scope = sa.Column(sa.Unicode(1000), nullable=False)  # payload:scope  The value of the scope parameter is expressed as a list of space- delimited, case-sensitive strings.

    # this is some housekeeping for lineage tracking
    grant_type = sa.Column(sa.Unicode(32), nullable=True, )  # could be for a `user` or for the `client`
    original_grant_type = sa.Column(sa.Unicode(32), nullable=True, )  # could be for a `user` or for the `client`
...
class Developer_OAuth2Client_BearerToken(Base):
    ...
    refresh_token = sa.Column(sa.Unicode(255), nullable=True, unique=False)  # not unique because we may recycle these
    scope = sa.Column(sa.Unicode(1000), nullable=False)
    timestamp_created = sa.Column(sa.DateTime, nullable=False)
    timestamp_expires = sa.Column(sa.DateTime, nullable=False)  # based on `payload:expires_in
    timestamp_revoked = sa.Column(sa.DateTime, nullable=True)
