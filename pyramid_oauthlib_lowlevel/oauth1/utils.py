    ...
    def wrapper(endpoint, *args, **kwargs):
        ...
        if endpoint.catch_errors:
            try:
                ...
                error = MiscellaneousOAuth1Error(_message,  # description
                                                 wrapped_exception = exc,
                                                 status_code = _status_code,
                                                 )
                error.error = exc.error  # copy over the error code
                log.warning('%s(%s) -> MiscellaneousOAuth1Error(%s)' % (exc.__class__.__name__, exc, _message))
                if PRINT_ERRORS:
                    print('%s(%s) -> MiscellaneousOAuth1Error(%s)' % (exc.__class__.__name__, exc, _message))
                if exc.wrapped_exception:
                    ...
                    if isinstance(exc.wrapped_exception, OAuth1Error):
                        ...
                        _status_code = exc.wrapped_exception.status_code  # copy over the error
